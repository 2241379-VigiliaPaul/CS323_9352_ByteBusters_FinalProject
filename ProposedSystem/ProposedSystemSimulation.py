...
TABLES_DIR = BASE_DIR / "ProposedSystem" / "TablesProposedSystem"
REPORT_DIR = BASE_DIR / "ProposedSystem" / "TextReportProposedSystem"
...
def _write_proposed_siman_report(
    ...
    r_scale_pct = (1 - run_scale) * 100
    d_scale_pct = (1 - dwell_scale) * 100
    ...
    lines.append("                             Project: Route 654 Proposed System")
    if replications == 1:
    ...
    lines.append("")
    lines.append("Proposed System Interventions")
    lines.append("-------------------------------------------------------------------------------")
    lines.append(f"Run-Time Scaling Factor             {run_scale:.2f}  ({r_scale_pct:.0f}% reduction)")
    lines.append(f"Dwell-Time Scaling Factor           {dwell_scale:.2f}  ({d_scale_pct:.0f}% reduction)")
    lines.append("Interpretation                     Replication-based discrete-event simulation under")
    lines.append("                                   fixed-demand (observed arrival) structure. Trip")
    lines.append("                                   schedules, arrival times, routing, and bus-to-trip")
    lines.append("                                   assignments are unchanged. Only service-time")
    lines.append("                                   components (run-time and dwell-time distributions)")
    lines.append("                                   are multiplicatively scaled as the experimental")
    ...
    lines.append("Note                               Non-congested — fixed demand, service-time")
    lines.append("                                   reduction only (no structural or demand-side")
    ...
    lines.append("-------------------------------------------------------------------------------")
    lines.append("Arrival Process                    Observed trip start times (unchanged)")
    lines.append("Run-Time Distribution              Empirical (by direction, segment) scaled by")
    lines.append(f"                                   {run_scale:.2f}")
    lines.append("Dwell-Time Distribution            Empirical (by direction, stop) scaled by")
    lines.append(f"                                   {dwell_scale:.2f}")
    ...
    lines.append(f"Queue Model Status                 Non-congested — fixed demand, service-time")
    lines.append(f"                                   reduction only (no structural or demand-side")
    ...
    lines.append("*******************************************************************************")
    lines.append("")
    lines.append("MODEL INTERPRETATION AND REPORTING CONSTRAINTS")
    lines.append("-------------------------------------------------------------------------------")
    lines.append("")
    lines.append("The simulation operates under a finite-horizon, replication-based discrete-event")
    lines.append("framework with fixed demand (observed trip schedules) and invariant routing and")
    lines.append("assignment structure. All arrivals, trip sequences, and bus-to-trip allocations")
    lines.append("are unchanged across scenarios. Experimental variation is applied strictly through")
    lines.append("multiplicative scaling of service-time components only (run-time and dwell-time")
    lines.append("distributions).")
    lines.append("")
    lines.append("The system must be interpreted as capacity-overprovisioned unless explicitly")
    lines.append("calibrated otherwise. Consequently:")
    lines.append("")
    lines.append("  - Queue lengths (Lq) and waiting times (Wq) are expected to be approximately")
    lines.append("    zero and must not be interpreted as indicators of system efficiency or")
    lines.append("    congestion.")
    lines.append("  - Resource utilization values must be treated as proxy estimates derived from")
    lines.append("    aggregated service times, not exact stochastic busy-time measurements.")
    lines.append("  - Throughput is a deterministic consequence of fixed demand and simulation")
    lines.append("    horizon, not a capacity outcome, and must not be used as evidence of system")
    lines.append("    improvement.")
    lines.append("")
    lines.append("All performance differences (e.g., MAE, RMSE, MAPE reductions) must be interpreted")
    lines.append("strictly as travel-time calibration effects resulting from service-time scaling,")
    lines.append("not as emergent network or capacity improvements.")
    lines.append("")
    lines.append("Replication outputs must be aggregated using:")
    lines.append("  - weighted means across replications for central tendency")
    lines.append("  - per-replication means for variance estimation and confidence intervals")
    lines.append("")
    lines.append("The model must explicitly avoid interpreting results as steady-state queueing")
    lines.append("behavior. Instead, all outputs represent a finite-horizon observational comparison")
    lines.append("under a controlled counterfactual scenario.")
    lines.append("")
    lines.append("Validity Constraint Statement")
    lines.append("  Results are valid only under fixed demand, overcapacity conditions, and")
    lines.append("  service-time-only interventions, and are not generalizable to congested or")
    lines.append("  demand-responsive systems.")
    lines.append("")
    lines.append("*******************************************************************************")
    lines.append("")
...
def _write_proposed_tables(
    ...
    validation_table = pd.DataFrame(
        [
            ...
            {"Metric": "Proposed mean duration", "Value": mean_sim, "Units": "mins"},
    ...
    validation_csv = output_dir / "proposed_table_validation.csv"
    validation_txt = output_dir / "proposed_table_validation.txt"
    ...
    distribution_table = pd.DataFrame(
        [
            ...
            dist_row("Proposed", results_df["simulated_duration_mins"]),
        ]
    )
    distribution_csv = output_dir / "proposed_table_distribution.csv"
    distribution_txt = output_dir / "proposed_table_distribution.txt"
    ...
    if rep_metrics:
        ...
        replication_csv = output_dir / "proposed_table_replications.csv"
        replication_txt = output_dir / "proposed_table_replications.txt"
    ...
    if baseline_path.exists():
        try:
            ...
            bl_values = dict(zip(bl_df["Metric"], bl_df["Value"]))
            bl_mean = bl_values.get("Simulated mean duration", float("nan"))
            bl_mae = bl_values.get("Mean Absolute Error", float("nan"))
            bl_rmse = bl_values.get("Root Mean Squared Error", float("nan"))
            bl_mape = bl_values.get("Mean Absolute Percentage Error", float("nan"))
            bl_tput = bl_values.get("Throughput", float("nan"))
            bl_lq = bl_values.get("Avg Queue Length (Lq)", float("nan"))
            bl_wq = bl_values.get("Avg Waiting Time (Wq)", float("nan"))
        except Exception:
            bl_mean = bl_mae = bl_rmse = bl_mape = bl_tput = bl_lq = bl_wq = float("nan")
    else:
        bl_mean = bl_mae = bl_rmse = bl_mape = bl_tput = bl_lq = bl_wq = float("nan")

    pct_change_mean = ((mean_sim - bl_mean) / bl_mean * 100) if pd.notna(bl_mean) and bl_mean else float("nan")
    pct_change_mae = ((mean_mae - bl_mae) / bl_mae * 100) if pd.notna(bl_mae) and bl_mae else float("nan")

    comparison_table = pd.DataFrame(
        [
            {"Metric": "Mean Trip Duration (mins)", "Baseline": f"{bl_mean:.4f}" if pd.notna(bl_mean) else "N/A", "Proposed": f"{mean_sim:.4f}", "Change": f"{pct_change_mean:+.2f}%" if pd.notna(pct_change_mean) else "N/A"},
            {"Metric": "Mean Absolute Error (mins)", "Baseline": f"{bl_mae:.4f}" if pd.notna(bl_mae) else "N/A", "Proposed": f"{mean_mae:.4f}", "Change": f"{pct_change_mae:+.2f}%" if pd.notna(pct_change_mae) else "N/A"},
            {"Metric": "RMSE (mins)", "Baseline": f"{bl_rmse:.4f}" if pd.notna(bl_rmse) else "N/A", "Proposed": f"{mean_rmse:.4f}", "Change": ""},
            {"Metric": "MAPE (%)", "Baseline": f"{bl_mape:.4f}" if pd.notna(bl_mape) else "N/A", "Proposed": f"{mean_mape:.4f}", "Change": ""},
            {"Metric": "Throughput (trips/hr)", "Baseline": f"{bl_tput:.4f}" if pd.notna(bl_tput) else "N/A", "Proposed": f"{mean_throughput:.4f}", "Change": ""},
            {"Metric": "Avg Queue Length (Lq)", "Baseline": f"{bl_lq:.4f}" if pd.notna(bl_lq) else "N/A", "Proposed": f"{mean_lq:.4f}", "Change": ""},
            {"Metric": "Avg Waiting Time (Wq mins)", "Baseline": f"{bl_wq:.4f}" if pd.notna(bl_wq) else "N/A", "Proposed": f"{mean_wq:.4f}", "Change": ""},
        ]
    )
    comparison_csv = comparison_dir / "comparison_summary.csv"
    comparison_txt = comparison_dir / "comparison_summary.txt"
    comparison_table.to_csv(comparison_csv, index=False)
    comparison_txt.write_text(
        comparison_table.fillna("N/A").to_string(index=False) + "\n",
        encoding="utf-8",
    )

    with open(comparison_txt, "a", encoding="utf-8") as f:
        f.write("\n")
        f.write("COMPARISON INTERPRETATION (MODEL-GRADE SPECIFICATION)\n")
        f.write("-------------------------------------------------------------------------------\n")
        f.write("\n")

        mae_up = pd.notna(mean_mae) and pd.notna(bl_mae) and mean_mae > bl_mae
        rmse_up = pd.notna(mean_rmse) and pd.notna(bl_rmse) and mean_rmse > bl_rmse
        mape_up = pd.notna(mean_mape) and pd.notna(bl_mape) and mean_mape > bl_mape
        mae_dn = pd.notna(mean_mae) and pd.notna(bl_mae) and mean_mae < bl_mae
        rmse_dn = pd.notna(mean_rmse) and pd.notna(bl_rmse) and mean_rmse < bl_rmse
        mape_dn = pd.notna(mean_mape) and pd.notna(bl_mape) and mean_mape < bl_mape

        all_improved = mae_dn and rmse_dn and mape_dn
        dur_decreased = pd.notna(mean_sim) and pd.notna(bl_mean) and mean_sim < bl_mean

        fmt_val = lambda v: f"{v:.4f}" if pd.notna(v) else "N/A"

        f.write("0. System Context\n")
        f.write("\n")
        f.write(
            "This comparison evaluates baseline vs proposed simulation under a "
            "fixed-demand, replication-based discrete-event system. All structural "
            "elements are invariant:\n"
        )
        f.write("\n")
        f.write("  Trip schedules (fixed demand)\n")
        f.write("  Network structure and routing logic\n")
        f.write("  Fleet size and resource allocation\n")
        f.write("\n")
        f.write("The only intervention is:\n")
        f.write("\n")
        f.write(
            f"  Multiplicative scaling of service-time distributions\n"
            f"  (run-time = {run_scale:.2f}, dwell-time = {dwell_scale:.2f})\n"
        )
        f.write("\n")

        f.write("1. Evaluation Variables\n")
        f.write("\n")
        f.write("Let:\n")
        f.write("\n")
        f.write("  MAE\u2080, RMSE\u2080, MAPE\u2080 = baseline metrics\n")
        f.write("  MAE\u2081, RMSE\u2081, MAPE\u2081 = proposed metrics\n")
        f.write("\n")
        f.write("Define deltas:\n")
        f.write("\n")
        f.write("  \u0394MAE = MAE\u2081 \u2212 MAE\u2080\n")
        f.write("  \u0394RMSE = RMSE\u2081 \u2212 RMSE\u2080\n")
        f.write("  \u0394MAPE = MAPE\u2081 \u2212 MAPE\u2080\n")
        f.write("\n")

        f.write("2. Calibration Decision Rule (Hard Constraint)\n")
        f.write("\n")
        f.write("Calibration status is defined as:\n")
        f.write("\n")
        f.write("  IMPROVED if:\n")
        f.write("    \u0394MAE < 0 AND \u0394RMSE < 0 AND \u0394MAPE < 0\n")
        f.write("  DEGRADED otherwise\n")
        f.write("\n")
        f.write("This rule is deterministic and overrides all secondary interpretations.\n")
        f.write("\n")

        f.write("3. Observed Values\n")
        f.write("\n")
        f.write(
            f"  MAE:  {fmt_val(bl_mae)} \u2192 {fmt_val(mean_mae)} "
            f"(\u0394 {'>' if mae_up else '<' if mae_dn else '='} 0)\n"
        )
        f.write(
            f"  RMSE: {fmt_val(bl_rmse)} \u2192 {fmt_val(mean_rmse)} "
            f"(\u0394 {'>' if rmse_up else '<' if rmse_dn else '='} 0)\n"
        )
        f.write(
            f"  MAPE: {fmt_val(bl_mape)} \u2192 {fmt_val(mean_mape)} "
            f"(\u0394 {'>' if mape_up else '<' if mape_dn else '='} 0)\n"
        )
        f.write("\n")

        f.write("4. Final Classification Output\n")
        f.write("\n")
        if all_improved:
            f.write("  Calibration Status: IMPROVED\n")
            f.write("  Trigger Condition: all error metrics decreased (\u0394 < 0)\n")
        else:
            f.write("  Calibration Status: DEGRADED\n")
            f.write("  Trigger Condition: at least one error metric increased (\u0394 \u2265 0)\n")
        f.write("\n")

        f.write("5. Bias Attribution\n")
        f.write("\n")
        f.write(
            f"The reduction in mean simulated trip duration "
            f"({fmt_val(bl_mean)} \u2192 {fmt_val(mean_sim)}) "
            f"is caused exclusively by service-time scaling.\n"
        )
        f.write("\n")
        f.write("This is classified as:\n")
        f.write("\n")
        f.write("  systematic downward bias in travel-time estimation\n")
        f.write("  not a structural or operational improvement\n")
        f.write("\n")

        f.write("6. Structural Invariance Assumption\n")
        f.write("\n")
        f.write("The following are constant:\n")
        f.write("\n")
        f.write("  Demand process\n")
        ...
        f.write("\n")
        f.write(
            f"  Throughput (~{fmt_val(mean_throughput)} trips/hr) is structurally fixed\n"
        )
        f.write("  Queue metrics (Lq = 0, Wq = 0) indicate a non-congested regime\n")
        f.write("  Queue metrics are not valid performance discriminators\n")
        f.write("\n")

        f.write("7. Metric Priority Order\n")
        f.write("\n")
        f.write("  1. Calibration error metrics (MAE, RMSE, MAPE) \u2014 primary decision basis\n")
        f.write(
            f"  2. Mean duration bias \u2014 "
            f"{'decreased' if dur_decreased else 'increased'} \u2014 "
            f"secondary diagnostic only\n"
        )
        f.write("  3. Throughput \u2014 invariant structural output\n")
        f.write("  4. Queue metrics \u2014 non-informative under overcapacity conditions\n")
        f.write("\n")

        f.write("8. Validity Constraint\n")
        f.write("\n")
        f.write("This interpretation is valid only under:\n")
        f.write("\n")
        f.write("  fixed demand\n")
        f.write("  service-time-only perturbations\n")
        f.write("  overcapacity / non-congested system behavior\n")
        f.write("\n")
        f.write("It is invalid for:\n")
        f.write("\n")
        f.write("  congested systems\n")
        f.write("  demand-responsive systems\n")
        f.write("  structural redesign scenarios\n")
...
def run_proposed_simulation(
    ...
    replications: int = 10,
    run_scale: float = 0.90,
    dwell_scale: float = 0.85,
) -> None:
    try:
        import simpy
    ...
    results_path = TABLES_DIR / "proposed_simulation_results.csv"
        ...
        [
            {
                ...
                "proposed_mean_mins": summary_metrics["sim_mean"],
                "mae_mins": summary_metrics["mae"],
                "rmse_mins": summary_metrics["rmse"],
            }
        ]
    )
    summary_path = TABLES_DIR / "proposed_simulation_summary.csv"
    summary.to_csv(summary_path, index=False)

    report_path = REPORT_DIR / "proposed_simulation_siman_report.txt"
    avg_run_length_hours = (
        sum(metric["run_length_hours"] for metric in rep_metrics) / len(rep_metrics)
        if rep_metrics
        else 0.0
    )
    _write_proposed_siman_report(
        results_df,
        trips_df,
        seed_base,
        replications,
    ...
    baseline_path = BASE_DIR / "CurrentSystem" / "TablesCurrentSystem" / "current_table_validation.csv"

    comparison_dir = BASE_DIR / "Comparison" / "Tables"
    comparison_dir.mkdir(parents=True, exist_ok=True)
    _write_proposed_tables(
        results_df,
        rep_metrics,
        TABLES_DIR,
        baseline_path,
    ...
    print("Proposed system simulation complete. Results saved to:", results_path)
    print(summary.to_string(index=False))


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the proposed system simulation with parametric improvements.")
    parser.add_argument(
        "--seed",
        type=int,
        default=42,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.90,
        help="Run-time scaling factor (e.g., 0.90 = 10%% reduction).",
    )
    parser.add_argument(
        "--dwell-scale",
        type=float,
        default=0.85,
        help="Dwell-time scaling factor (e.g., 0.85 = 15%% reduction).",
    )
